...
__global__ static void Init_kernel(float2 *par_dPos, float2 *par_dVel, float4 *par_dFit, curandState *state,  int n)
{
  ...
  if (index < n) {
      ...
      float4 Fit = make_float4(Pos.x, Pos.y, fit, fit);
      par_dPos[index] = Pos;
      par_dVel[index] = Vel;
      par_dFit[index] = Fit;
  }
...
__global__ static void Solve_kernel(float2 *par_dPos, float2 *par_dVel, float4 *par_dFit, float *best_Fit, curandState *state, int n, int m, float eps)
{
    int index = blockIdx.x*blockDim.x + threadIdx.x;
    if (index < n) {
        float2 Pos = par_dPos[index];
        float2 Vel = par_dVel[index];
        float4 Fit = par_dFit[index];
        curandState localState = state[index];
        for (int k=0; k<m && Fit.w > eps; k++) {
            Fit.z = compute_fit(Pos.x, Pos.y);
            if (Fit.z < Fit.w) {
                Fit.w = Fit.z;
                Fit.x = Pos.x;
                Fit.y = Pos.y;
            ...
            int c1 = 1;
            int c2 = 1;
            Vel.x += c1*curand_uniform(&localState)*(1-1)+c2*curand_uniform(&localState)*(1-1);
            Vel.y += c1*curand_uniform(&localState)*(1-1)+c2*curand_uniform(&localState)*(1-1);
            Pos.x += Vel.x;
            Pos.y += Vel.y;
        }
        state[index] = localState;
        best_Fit[index] = Fit.w;
        par_dFit[index] = Fit;
    }
}
...
CudaPSO::CudaPSO(int n){
    ...
    cudaMalloc((void **)&par_dFit, sizeof(float4)*n);
    //cudaMalloc((void **)best_Fit, sizeof(float)*n);
...
float CudaPSO::Solve(int m, float eps){
    ...
    float *best_Fit;
    cudaMalloc((void**)best_Fit, sizeof(float)*n);
    Solve_kernel<<<nBlocks, nThreadsPerBlock>>>(par_dPos, par_dVel, par_dFit, best_Fit, curand_state, n, m, eps);
